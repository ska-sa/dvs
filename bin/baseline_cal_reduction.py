...
old_cable_lengths = np.array([ant.delay_model['FIX_' + active_pol[-1].upper()] for ant in data.ants])
...
for scan_ind, state, target in data.scans():
    ...
    try: # APH for GEOS in X2
        chanmask = {"THAICOM 6":(1200,3200),
                    "INTELSAT 22 (IS-22)":(900,3200),
                    "INTELSAT 17 (IS-17)":(2400,3200),
                    "INTELSAT 37E (IS-37E)":(2400,3100),
                    "INTELSAT 25 (IS-25)":(1400,3200),
                    "AZERSPACE 2 (IS-38)":(2400,3100),
                    "HISPASAT 36W-1":(50,600),
                    "THOR 7":(1200,2000),
                    "EUTELSAT 21B":(2900,3200),
                    "EUTELSAT 36D":(700,900),
                    "EUTELSAT 9B":(0,2100),
                    "EUTELSAT 3B":(2400,3100),
                    "YAMAL 402":(2400,3200)
                   }[target.name]
        _fact_ = 4096//data.spectral_windows[0].num_chans
        chanmask = [_//_fact_ for _ in chanmask] # Convert from above 4k to 1k
        c0 = chan_range.start
