...
# Also includes _circle_ pattern which simply scans along a fixed radial offset from the target, to determine pointing offsets while tracking [aph@ska.ac.za 11/2021].
#   NB: _circle_ pattern must also scan across bore sight to constrain beam amplitude, TODO: change to cardioid?
...
def generatepattern(totextent=10,tottime=1800,tracktime=5,slowtime=6,sampletime=1,scanspeed=0.15,slewspeed=-1,twistfactor=1.,trackinterval=1,kind='spiral'):
    ...
    elif kind=='_circle_':# Scan along a constant radial offset from target centre
...
if __name__=="__main__":
    ...
    if testmode:
        ...
        if len(args)==0 or args[0]=='lbandtargets':#lband targets, in order of brightness
            ...
            args=['3C 454.3','PKS 0723-008','3C 279','PKS 2134+004','PKS 1421-490','PKS 1934-63','PKS 0408-65']
                ...
                while cycle<opts.num_cycles or opts.num_cycles<0: # Override exit conditions are coded in the next ten lines
                    ...
                    for igroup in grouprange:
                        ...
                        if target is None:
                            ...
                            if (igroup == max(grouprange)): targets.remove(target)
