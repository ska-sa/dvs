...
def process_a(h5, ant, freq_min=0, freq_max=20e9, channel_bw=10e6, sky_radius=30, freq_mask='', debug=False):
    """ @param freq_min, freq_max: range of frequencies to process [Hz] (default 0, 20e9)
        ...
        @param freq_mask: either a pickle file of boolean flags for the dataset's frequency channels,
                              or a text file with each line frequency start,stop to indicate omitted range.
        @return: (nice_title,T_SysTemps,freq_list,elevation,ra,dec,surface_temperature,air_relative_humidity,receiver,SpillOver,aperture_efficiency) - freq_list is in MHz """
...
def process(dataset, ant, f_min, f_max, freq_mask='', apscale=1.,PLANE="antenna",spec_sky=True,MeerKAT=True):
    """ Load and process the tipping curve data file, to generate the information from which a report can be compiled.
        ...
        @return: (nice_title, freq_list, T_SysTemp, aperture_efficiency, tsys, tant, PLANE) - freq_list is in MHz """
...
def report(ant, filename, nice_title, freq_list, T_SysTemp, aperture_efficiency, tsys, tant, PLANE, select_freq=None, select_el=[15,45,90], cartesian=True, plot_limits=False):
    """ Generate the standard tipping curve report.
        @param ant, filename: string labels for the report
        @param nice_title, freq_list, T_SysTemp, aperture_efficiency, tsys, tant, PLANE: as generated by `process()`
