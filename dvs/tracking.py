...
def reduce_pointing_scans(ds, ant, chans=None, freq_MHz=None, track_ant=None, phased_up=False, flags='cam,data_lost', scans="~slew", compscans="~slew",
    ...
    ant_type = 'e' if ((ant[0] == 's') and (ds.timestamps[-1] < 1756490000)) else ant[0] # Determine whether it's MKE(prior to 28/08/2025) or SKA
    ...
    for (cs_no, cs_label, target) in ds.compscans():
        ...
        if (len(ds.timestamps[mask]) < min_len): # All points flagged out
            continue
