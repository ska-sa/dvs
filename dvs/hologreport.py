...
def e_bn(pol, tilt_deg=0, obs_lon=21.4438888,obs_lat=-30.7110555):
    """ Generates the linear E-field components for a polarised signal arriving from space,
        ...
        tilt angle from the surface of the Earth in the northern hemisphere is positive in the sense that
        it rotates +H to +V. Satellite tilt angles either from e.g. https://www.satbeams.com/footprints
        ...
        Following the IAU mapping, test cases for a northern observer are:
            ...
            H@90deg tilt should equal V@0deg tilt
        Test cases for a southern observer:
            V@0deg tilt should be [-1,0]; H@0deg should be [0,-1]
            H@90deg tilt should equal V@0deg tilt
        
        @param pol: may be one of 'RCP'|'LCP'|'H'|'V', or else the un-tilted components (e_X, e_Y)
    ...
    if (len(np.atleast_1d(tilt_deg)) == 2): # (sat lon, extra tilt) to calculate skew angle for GEO satellite
        ...
        # Skew for observer in northern hemisphere to be consistent with extra_tilt; convert to south later
        skew = np.atan(np.sin((obs_lon-sat_lon)*D2R)/np.tan(abs(obs_lat)*D2R))
    ...
    return [np.round(_, 4) for _ in [X*np.cos(tilt)+Y*np.sin(tilt), -X*np.sin(tilt)+Y*np.cos(tilt)]]
...
def check_timingoffset(fn, freqMHz, ant, timingoffset=0, cycle=(0,1), dMHz=0.1, extent=None, telescopename="SKA"):
    ...
    for f in freqMHz:
        if (extent is not None):
            ...
            fig = plt.figure(figsize=(8*max(2,len(ds)),5-min(2,len(ds))))
...
HologResults = namedtuple("HologResults",["fid","el_deg","f_MHz","feedoffsetsH","feedoffsetsV","rpeffH","rpeffV",
                                          "rmsH","rmsV","errbeamH","errbeamV","info","tags","cycles"])
...
def _unsqueeze_(rs):
    ...
    if isinstance(rs.el_deg, float):
        ...
        rs = HologResults(rs.fid, nest1d(rs.el_deg), rs.f_MHz, nest3d(rs.feedoffsetsH), nest3d(rs.feedoffsetsV),
                          nest3d(rs.rpeffH), nest3d(rs.rpeffV), nest2d(rs.rmsH), nest2d(rs.rmsV), nest3d(rs.errbeamH), nest3d(rs.errbeamV),
                          dict([(k,nest1d(v)) for k,v in rs.info.items()]), rs.tags, rs.cycles)
    return rs
...
def plot_errbeam_against(RS, labels, key, fit=None, eval_fit_at=None, extra="95pct", figsize=(14,4), ylim=(0,10), hold=False):
    ...
    fit_args = dict(order=1, method={'lin':'leastsq'}.get(fit,fit)) if fit in ["lin","leastsq","theil-sen"] else ({} if (fit is None) else dict(**fit))
...
def standard_report(measured, predicted=None, DF=5, spec_freq_MHz=[15000,20000], tzoffset=2, contourdB=-20, beampolydegree=28, beamsmoothing='fourier', eb_extent=(-0.2,0.2), coords="SKA", debug=False, makefigs=True, makepdf=True, pdfprefix="", **devkwargs):
    ...
    try:
        ...
        results = HologResults(measured.fid, el_deg, measured.f_MHz, np.ma.masked_array(feedoffsetsH, fill_value=np.nan), np.ma.masked_array(feedoffsetsV, fill_value=np.nan),
                               ...
                               dict(time_hod=time_hod,deg_per_sec=deg_per_sec,feedindexer_deg=feedindexer_deg,enviro=enviro), measured.tags, measured.cycles)
        
        if (ci > 0) and (makefigs or makepdf): # Multiple cycles
            plot_vs_hod([[results]], ["%d"%measured.fid], False, spec_freq_MHz, figsize=(14,4))
            pp.report_fig(max(plt.get_fignums()))
            if (_predicted_ is not None):
                plot_errbeam_cycles([measured], predicted, DF=DF, beampolydegree=beampolydegree, beamsmoothing=beamsmoothing, contourdB=contourdB, extent=eb_extent,
                                    ncols=6, clim=[i*np.nanmedian(results.errbeamH[:,0]) for i in [-1,1]])
                pp.report_fig(max(plt.get_fignums()))
    finally:
        pp.close()
...
def generate_results(rec, predicted=None, mask_xlin=2, SNR_min=30, phaseRMS_max=30, clim=(-0.8,0.8), eb_extent=(-0.2,0.2), makefigs=True, makepdfs=False, pdfprefix="ku", **report_kwargs):
    """ Generate reports with 'standard_report()' and returns the results cross-indexed by all defined tags.
...
def mask_results(r, rec, mask_xlin=2, SNR_min=30, phaseRMS_max=30):
    """ Apply masks to the results where the specified criteria are not met.
        Result fields 'feedoffsetsHV' & 'errbeamHV' are numpy masked arrays, with masked values represented by NaN's.
        Aperture-plane efficiency & RMS seem to be robust against factors filtered for, therefore not masked.
        
        @param r: a HologResults record
        @param rec: a ResultSet record with data that's already been loaded & processed.
        @param mask_xlin: mask out feedoffsets & errorbeam where beacon yields linear polarisation suppressed by >= this factor (default 2; 3 still leaves suppressed pol with large scatter in offsets)
        @param SNR_min: a cycle for which the co-pol amplitude SNR (mean/stddev) is below this threshold over the cycle, will be masked out (default 30)
        @param phaseRMS_max: a cycle for which the co-pol phase scatter (relative to piecewise linear) is above this threshold is masked as 'True' (default 30).
        @return: `r`, modified in-place!
    """
        # TODO: HologResults needs to carry: fid, f_MHz, beacon_pol, snr, tags, cycles
    ...
    for i,(pol,bms) in enumerate(zip(rec.beacon_pol, rec.beams)):
        ...
        if linH:
            ...
            maskH[-1][:] = True
    if np.any(maskH) or np.any(maskV):
        print("\n")
        ...
        for r in rr: # Select individual measurements in each HologResults based on frequency
            ...
            if (len(f_MHz) > 0):
                ...
                def masked(cd):
                    ...
                    return ma
                hr = HologResults(fid=r.fid, el_deg=r.el_deg,f_MHz=f_MHz,feedoffsetsH=masked(feedoffsetsH),feedoffsetsV=masked(feedoffsetsV),
...
def recalc_eff(apmapsX, apmapsY, freqs_MHz, D=None, save_to=None, band=""):
    """ Re-computes efficiencies for the physical geometric aperture. For MeerKAT this is as masked for ApertureMap.devmap,
        ...
        @param save_to: specify this folder to save results to a file that can be used with katsemodels.py (the labeling assumes that X=H and Y=V).
...
def calculate_ant_eff(mss, rrs=None, taper=None, ieee=True, fspec_MHz=None, return_ids=False):
    """ Calculate antenna efficiency "As-Is", individually for each measurement in a set of holography measurements,
        at all frequencies defined for the datasets.
        Results are computed as per IEEE Std 145:
          antenna_efficiency (IEEE ANTENNA APERTURE ILLUMINATION EFFICIENCY) includes all effects that impact on directivity pattern
        
        @param mss: list of ResultSets
        @param rrs: map of HologResults as generated by generate_results() - only used for masking (default None)
        @param taper: a positive fraction (voltage ratio at edge relative to center of aperture) to use a synthetic amplitude map,
                      or None to keep the measured amplitude map (default None)
        @param ieee: True to calculate efficiency as ratio of gains, False for taper*phase (default True)
        @param fspec_MHz: frequency to scale results to - only if ieee=False (default None)
        @return: (freq, ant_effHV, masksHV) in MHz & masked as per 'rrs:feedoffsets' """
    fids, freq, ant_eff, mask = [], [], [], []
    for ms in mss:
        # "Raw data"
        amH, amV, f = ms.apmapsH, ms.apmapsV, ms.f_MHz
        ...
        if (taper is not None):
            ...
            tapermap = amH.apmap # TODO
            amH, amH = copy.copy(amH), copy.copy(amV)
            amH.ampmap = amH.tapermap
            amV.ampmap = amV.tapermap
        
        # Calculate results
        if ieee: # aeff is IEEE ANTENNA APERTURE ILLUMINATION EFFICIENCY
            ill,aeff,Ag = recalc_eff(amH, amV, f)
        else: # aeff is phase*taper
            if np.isscalar(fspec_MHz): # Has no effect if ieee=True
                for a,b in zip(amH,amV):
                    a.gain(freqscaling=fspec_MHz*1e6/a.freq); b.gain(freqscaling=fspec_MHz*1e6/b.freq)
            aeff = np.array([[apmap[0].eff0_phase*apmap[0].eff_taper,
                              apmap[1].eff0_phase*apmap[1].eff_taper] for apmap in zip(amH,amV)])*100
            # aeff *= .88 # Add approximate spillover efficiency
            if np.isscalar(fspec_MHz):
                for a,b in zip(amH,amV):
                    a.gain(freqscaling=1); b.gain(freqscaling=1)
        
        msk = np.c_[maskH, maskV]
        fids.extend([ms.fid]*len(f)); freq.extend(f); ant_eff.extend(aeff), mask.extend(msk)
    
    if not return_ids:
        return np.array(freq), np.array(ant_eff), np.array(mask)
    else:
        return np.array(freq), np.array(ant_eff), np.array(mask), np.array(fids)
